...
public class GetSessionsWithMovieCommandHTMLView extends CommandViewHTML {
    ...
    private static final String CSS_STYLES = ".centered {\n" +
            ...
            "text-align: center;\n" +
            "}\n";
    private static final String DEFAULT_CONTENT = "<h1 class='not_found'>No Sessions were found!</h1>\n";
    ...
    public String getContent(CommandResult commandResult) {
        ...
        for (Session s : sessions)
            result.append("<tr class='hover_content'>\n<td><a href='/cinemas/").append(s.getTheater().getCinema().getId())
                    .append("/theaters/").append(s.getTheater().getId()).append("/sessions/").append(s.getId()).append("'>")
                    .append(s.getId()).append("</a></td>\n").append("<td><a href='/cinemas/").append(s.getTheater().getCinema().getId())
                    .append("'>").append(s.getTheater().getCinema().getName()).append("</a></td>\n").append("<td>").append(s.getDate())
                    .append("</td>\n").append("</tr>\n");
        result.append("</table>\n");
